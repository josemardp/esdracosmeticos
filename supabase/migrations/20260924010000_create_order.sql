-- ============================================================
-- MIGRATION: 20260924010000_create_order.sql
-- Objetivo: Criar em produção a RPC create_order que o checkout chama.
-- Ela nunca tinha sido aplicada (0 pedidos até 23/09/2026).
--
-- Base: versão de 20260321000000_create_order_server_side.sql, com ajustes:
--   - erro claro se algum item do carrinho não existe ou está inativo
--   - quantidade nula tratada como inválida
--   - uso do cupom por v_coupon_id (ler v_coupon.id sem cupom dava erro
--     "record is not assigned yet" em todo pedido sem cupom)
--   - removidos os REVOKEs de increment_coupon_usage e
--     process_order_inventory_and_coupon, que não existem em produção
-- As migrações 20260321000000 e 20260321202944 nunca foram aplicadas;
-- esta é a definição que vale.
-- ============================================================
...
DECLARE
  ...
  v_coupon         RECORD;
  v_coupon_id      UUID;  -- v_coupon fica sem valor quando não há cupom
  v_item           RECORD;
  v_product        RECORD;
BEGIN
  ...
  -- Todo item precisa apontar para um produto ativo (senão o pedido
  -- sairia sem aquele item, sem o cliente saber)
  IF EXISTS (
    SELECT 1
    FROM jsonb_to_recordset(p_items) AS x(product_id UUID, qty INT)
    LEFT JOIN public.products p ON p.id = x.product_id AND p.active = true
    WHERE p.id IS NULL
  ) THEN
    RAISE EXCEPTION 'Produto não encontrado ou indisponível no carrinho';
  END IF;
...
-- O frontend não chama decrement_inventory direto; só esta função.
REVOKE EXECUTE ON FUNCTION public.decrement_inventory(UUID, INT) FROM PUBLIC, anon;
